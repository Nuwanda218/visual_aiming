        ...
        return [
            ...
            (
                ...
                [
                    ...
                    ParamSpec("fps_max_speed", "最大速度", 500, 12000, 100, "速度状态上限。限制远距离追赶的最高速度。"),
                    ParamSpec("fps_decel_radius", "减速半径", 10, 300, 1, "进入该范围后按距离明显降速。增大会更早减速。"),
                    ParamSpec("fps_near_speed_scale", "近场速度比例", 0.01, 0.8, 0.01, "非常接近目标时保留多少速度。降低更稳，提高更跟手。"),
                    ParamSpec("fps_brake_radius", "近场刹车半径", 5, 220, 1, "进入该范围后额外衰减速度惯性。"),
                    ParamSpec("fps_brake", "近场刹车强度", 0, 1.5, 0.01, "近场速度衰减强度。提高后更容易收住。"),
                    ParamSpec("fps_measurement_blend", "测量融合", 0.05, 1.0, 0.01, "新检测点修正内部误差的比例。高了更跟检测，低了更平滑。"),
                    ParamSpec("fps_reacquire_distance", "跳变重置距离", 20, 500, 5, "新检测距离变化超过该值时清空旧速度并重新吸附。"),
                    ParamSpec("fps_jitter_angle", "弹动角度", 0, 0.3, 0.005, "给目标方向加入轻微角度扰动。需要稳定时保持 0。"),
                    ParamSpec("fps_velocity_feedback", "速度反馈抵消", 0, 1.0, 0.01, "发送鼠标位移后对内部速度做反向修正。摆动时可小幅提高。"),
                ],
            ),
            (
                "预测跟踪",

...
def make_config(image_width=410, image_height=315):
    config = Config()
...
class VideoFileCaptureRoiTests(unittest.TestCase):
    ...
    def test_roi_clamps_to_video_size(self):
        ...
        config = make_config(image_width=9999, image_height=9999)
        cap = VideoFileCapture.__new__(VideoFileCapture)
        cap.video_width = 640
        cap.video_height = 480
        cap._roi_w = min(config.capture.image_width, cap.video_width)
        cap._roi_h = min(config.capture.image_height, cap.video_height)
    ...
    def test_roi_centers_on_frame(self):
        """ROI 小于视频时应居中裁切。"""
        config = make_config(image_width=200, image_height=100)
        cap = VideoFileCapture.__new__(VideoFileCapture)
        cap.video_width = 640
        cap.video_height = 480
        cap._roi_w = min(config.capture.image_width, cap.video_width)
        ...
        self.assertEqual(cap._roi_left, 220)
        self.assertEqual(cap._roi_top, 190)
